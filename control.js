...
export async function main(ns) {
	...
	*/
	...
	const memoryRatio = 0.80; //how close to max memory should we get
	const moneyRatio = 0.05; //ratio of available money to hack
	...
	**/
	async function prepare(growAmount, weakenAmount) {
		...
		let gThreads = Math.max(Math.ceil(Math.min(ns.growthAnalyze(target.hostname, growMultiplier, source.cpuCores), availableMemory / workerMemory )), 1); //threads to grow to 100% money
		let wThreadsPost = Math.max(Math.ceil(Math.min(gThreads * growSecurity / weakenSecurity), availableMemory / workerMemory ), 1); //threads to weaken post-grow

		let requiredMemory = (wThreadsPre + gThreads + wThreadsPost) * workerMemory;
		
		if (requiredMemory / availableMemory > memoryRatio) {
			ns.print(`prep: ${target.hostname}\n  Low memory: ${f.formatBytes(availableMemory)}/${f.formatBytes(requiredMemory)}`);
		}
	...
	while (true) {
		...
		let gThreads = Math.max(Math.ceil(ns.growthAnalyze(target.hostname, growMultiplier, source.cpuCores)), 2); //threads to grow to 100% money (extra thread for insurance)
		...
		for (var i = 0; i < instances; i++) {
			...
			let availableMemory = source.maxRam - ns.getServerUsedRam(source.hostname);
			if (requiredMemory / availableMemory > memoryRatio) {
				ns.print(`unable to hack/grow: ${target.hostname}\n  insufficient memory: ${f.formatBytes(availableMemory)}/${f.formatBytes(requiredMemory)}`);
				break;
			}
